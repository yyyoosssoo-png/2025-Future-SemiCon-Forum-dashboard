...
<body>
    ...
    <div id="hrd" class="tab-content">
        <div class="container">
            ...
            <!-- Footer for HRD tab only -->
            <div style="background: rgba(20, 16, 36, 0.9); color: #C6C2F4; padding: 40px 20px; text-align: center; border-top: 2px solid rgba(119, 112, 230, 0.2); margin-top: 60px; border-radius: 15px;">
                <div style="font-size: 14px; font-style: italic; line-height: 1.8;">
            ...
            try {
                            ...
                            afterDatasetsDraw: function(chart) {
                                ...
                                chart.data.datasets.forEach((dataset, i) => {
                                    ...
                                    meta.data.forEach((point, index) => {
                                        ...
                                        ctx.fillStyle = '#EBE9FB';
                                        ctx.font = 'bold 14px Segoe UI';
                                        ctx.textAlign = 'center';
                                        ctx.textBaseline = 'middle';
                                        
                                        // Calculate position inside the polygon
                                        const angle = chart.scales.r.getPointLabelPosition(index, 10);
                                        const centerX = chart.scales.r.xCenter;
                                        const centerY = chart.scales.r.yCenter;
                                        const distance = chart.scales.r.getDistanceFromCenterForValue(value);
                                        
                                        // Position text slightly towards center (70% of the distance)
                                        const x = centerX + Math.cos(angle.angle) * distance * 0.7;
                                        const y = centerY + Math.sin(angle.angle) * distance * 0.7;
                ...
                if (barCtx) {
                    ...
                    new Chart(barCtx.getContext('2d'), {
                        ...
                        data: {
                            labels: [
                                ['Speech1 Marvell', 'Architecting the Next AI Chip'],
                                ['Speech2 AWS', 'Next Wave of AI Platform'],
                                ['Speech3 SK이노베이션', 'Energy for AI Computing Infra'],
                                ['Speech4 SK증권', 'Future Paradigms of AI Chips'],
                                ['Discussion1', 'Future of AI Architecture', '효율과 Scale-up의 최전선'],
                                ['Discussion2', 'beyond HBM', '메모리 반도체, Next를 위한 도약']
                            ],
                            datasets: [{
                                label: '평균 만족도',
